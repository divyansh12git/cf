...
void divyansh_8(){
    ...
    ll ans=0;
    bool cas=0;
    if(a[0]==pq.top() || a.back()==pq.top()){
        ans+=pq.top();
        pq.pop();
        cas=1;
    }
    if((!cas) && k==1){
        cout<<pq.top()+max(a[0],a.back())<<endl;
        return;
    }
    while(x<k){
        ll q=pq.top();
    ...
    // cerr<<ans<<endl;/
    // int p1=0,p2=n-1;
    // while(p1<n && st.find(a[p1])==st.end())p1++;
    // while(p2>=0 && st.find(a[p2])==st.end())p2--;
    // cerr<<p1<<" "<<p2<<endl;
    ll el=-1;
    fl(i,0,n){
    ...
    // if(p1>0)el=max(el,a[0]);
    // if(p2<n-1)el=max(el,a[n-1]);
    cout<<ans+el<<endl;

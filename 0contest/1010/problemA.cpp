...
typedef tree<pair<int,int>, null_type, less<pair<int,int>>, rb_tree_tag, tree_order_statistics_node_update> pbds; // find_by_order, order_of_key//ordered set
...
void divyansh_8(vector<int>& nums){
    //code here...    
    int n=nums.size();
    pbds pd1,pd2;
    ...
    pd2.insert({nums[1],0});
    for(int i=2;i<n;i++){
        int x=pd1.size()-pd1.order_of_key({nums[i],INT_MAX});
        int y=pd2.size()-pd2.order_of_key({nums[i],INT_MAX});
        if(x>y){
            pd1.insert({nums[i],i});
        }else if(y>x){
            pd2.insert({nums[i],i});
...
int main(){
    ...
    ll t; if(istc)cin>>t;else t=1; while(t--)divyansh_8(nums);

...
void divyansh_8(){
    ...
    ll n,m;
    cin>>n>>m;
    vpll cir(n);//{center,radii}
    fl(i,0,n)cin>>cir[i].F;
    fl(i,0,n)cin>>cir[i].S;
    vsort(cir);
    ll ans=0;
    vll cnt(n+1);
    cnt[0]=0;
    vi radii;
    fl(i,0,n){
        ll pt=2*((cir[i].S)*(cir[i].S))+2*cir[i].S+1;
        cerr<<cir[i].S<<" "<<pt<<endl;
        ll r=cir[i].S;
        cnt[i+1]=pt;
        if(i==0){
            cnt[i+1]=pt;
        ...
        auto lb=lower_bound(radii.begin(),radii.begin()+i,cir[i].F-r);
        cerr<<i<<" "<<lb-radii.begin()<<endl;
        if(lb!=radii.end()){
            int ind=lb-radii.begin();
            pt-=cnt[i]-cnt[ind]; 
            if(cir[i].F-cir[i].S>cir[ind].F-cir[ind].S){
                int dis=cir[ind].F+cir[ind].S-cir[i].F-r;
                if(dis>=0 && dis>r*r){
                    pt-=r*r;
                    dis-=r+1;
                    pt-=dis*dis+1;

...
ll getmaxiY(ll x,ll r, ll c){
    ll p=(lld)r*r-(x-c)*(lld)(x-c);
    p=sqrt(p);
    return p;
}
...
void divyansh_8(){
    ...
    map<ll,ll>mp;//for a corresponding x maximum value of y
    for(auto it:cir){
        for(int x=it.F-it.S;x<=it.F+it.S;x++){
            ll p=getmaxiY(x,it.S,it.F);
            if(mp.find(x)!=mp.end()){
                mp[x]=max(mp[x],p);
            }else mp[x]=p;
        }

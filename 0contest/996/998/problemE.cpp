...
int findParent(int u,vi& par){
    if(par[u]==u)return u;
    return par[u]=findParent(par[u],par);
}
int addVer(int u,int v,vi &par, vi&rank){
...
void solve(){
    ...
    fl(i,0,m2){
        ...
        adj2[u].insert(v);
        adj2[v].insert(u);
        addVer(u,v,parent2,rank);
    }
    int cost=0;
    fl(i,1,n+1){
        if(findParent(i,parent1)==findParent(i,parent2)){
            continue;
        }else{
            cost++;
            parent1[i]=i;

...
void solve(){
    ...
    fl(i,0,n)sort(deck[i].begin(),deck[i].end());
    vpii q;
    fl(i,0,n)q.pb({deck[i][0],i});
    vsort(q);
    vi ans;
    // _print(q);
    ans.pb(q[0].S);
    ...
    int las=deck[ans.back()][0];
    fl(i,1,m){
        fl(j,0,n){
                // cerr<<deck[ans[j]][i]<<endl;
                if(j==0){
                    if(deck[ans[j]][i]<=las){
                        cout<<-1<<endl;return;
                    }continue;
                }
            if(deck[ans[j]][i]<=deck[ans[j-1]][i]){
                cout<<-1<<endl;return;
            }
            if(j==n-1){
                las=deck[ans.back()][i];
    ...
    fl(i,0,n)cout<<ans[i]+1<<" ";cout<<endl;
}

...
void remove(vector<int>adj[],vector<int>&deg,vpii q,set<int>&del){
    int node=q[0].S;
    int maxdeg=q[0].F;
    bool dne=0;
    fl(i,0,q.size()){
...
void solve(){
    ...
    reverse(all(q2));
    // _print(q2);
    remove(adj,deg,q2,del);
    DisjointSet ds(n);
    fl(i,1,n+1){
        if(del.find(i)!=del.end())continue;
        for(auto nei:adj[i]){
            if(del.find(nei)!=del.end())continue;
            ds.unionByRank(i,nei);
        }
    }
    // _print(del);
    // _print(ds.parent);cerr<<endl;
    set<int>comp;
    fl(i,1,ds.parent.size()){
        comp.insert(ds.parent[i]);
    }

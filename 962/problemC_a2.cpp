...
void solve(){
    ...
    int n,q;cin>>n>>q;
    string a,b;cin>>a>>b;
    vector<vector<int>>pra(n+1,vector<int>(26,0));
    vector<vector<int>>prb(n+1,vector<int>(26,0));  
...
// debug(2);
    while(q--){
        ...
        // debug(55);
        // _print(pra[r]);_print("\n");
        // _print(pra[l-1]);_print("\n");
        fl(i,0,26){
            // debug(33);
            x[i]=pra[r][i]-pra[l-1][i];
            y[i]=prb[r][i]-prb[l-1][i];
        }
        _print(x);_print(y);_print("\n");
        int diff=0;
        int pos=0,neg=0;
        fl(i,0,26){
            if(x[i]-y[i]>0)pos+=x[i]-y[i];
            if(x[i]-y[i]<0)neg+=x[i]-y[i];

...
void solve(){
    ...
    int n,q;cin>>n>>q;
    string a,b;cin>>a>>b;
    vector<pair<map<char,int>,map<char,int>>>vec(n);
    // fl(i,0,n){
    //     cerr<<i<<endl;
    //     _print(vec[i].F);_print("\n");
    //     _print(vec[i].S);_print("\n");
    // }
    while(q--){
        int l,r;cin>>l>>r;
        map<char,int>counta;
        ...
        // _print(vec[r-1].F);_print(vec[l-1].F);
        // _print()
       fl(i,l-1,r){
            counta[a[i]]++;
            countb[b[i]]++;  
        }
        // _print(counta);_print("\n");_print(countb);_print("\n");
        int neg=0;int pos=0;
        for(auto i:counta){
            int c=i.S;char ch=i.F;
            int q=c;
            if(countb.find(ch)!=countb.end()){
                q-=countb[ch];
            }    
            if(q>=0)pos+=q;
            else neg+=q;

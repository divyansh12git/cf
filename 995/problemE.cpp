...
void solve(){
    ...
    map<int,int>mp;
    fl(i,0,n){
        if(mp.find(p[i].F)==mp.end())mp[p[i].F]=i;
    }
    // _print(mp);
    vsort(a);vsort(b);
    ll maxi=0;
    if(k==0){
        cout<<n*a[0]<<endl;return;
    }
    if(k>=n)maxi=1e18;
    else maxi=a[k];
    int p1=0;
    ll ans=-1e9;
    // if(a.back()>)
    while(p1<n && (maxi==1e18 || b[p1]<=maxi) ){
        ans=max(ans,(n-p1)*b[p1]);
        p1++;
    }
    if(ans==-1e9){
        for(int i=0;((i<n) && a[i]<=maxi);i++){
            ans=max(ans,(n-mp[i])*a[i]);
        }
        if(maxi==1e18)ans=b.back()*n;
    }
cout<<ans<<endl;

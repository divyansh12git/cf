...
int lcs(string &a,string &b,int x,int y,vector<vector<int>>dp,int &flen,int len){
    if(x<0 || y<0){flen=max(len,flen);return 0;}
    if(dp[x][y]!=-1)return dp[x][y];
    if(a[x]==b[y]){
        len++;
        return dp[x][y]=1+lcs(a,b,x-1,y-1,dp,flen,len);
    }
    flen=max(len,flen);
    len=0;
    return dp[x][y]=max(lcs(a,b,x,y-1,dp,flen,len),lcs(a,b,x-1,y,dp,flen,len));
}
...
void solve(){
    ...
    // int x=lcs(a,b,a.length()-1,b.length()-1,dp,flen,0);
    // cout<<flen<<endl;
    int count=0;
    fl(i,0,a.length()){
        fl(j,0,b.length()){
    ...
    // cout<<a.length()+b.length()-flen<<endl;
}

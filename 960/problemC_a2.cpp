...
void solve(){
    ...
    vector<ll>a(n);
    llfl(i,0,n)cin>>a[i];
    vector<pair<int,pair<int,int>>>count;
    map<int,int>mp;
    int maxi=0;
    ll sum=0;
    for(int i=0;i<n;i++){
        ...
        if(mp[a[i]]>=2){
            if(a[i]>maxi){
                ...
                if(count.size()>0)count.back().S.S++;
            }
        }else{
            if(count.size()!=0){
                count.back().S.S++;
    ...
    for(int i=0;i<count.size();i++){
        if(count[i].S.S==1){
            sum+=count[i].S.F;
            if(i-1>=0){
                sum+=(count[i-1].S.F)*(n-(count[i].S.S+count[i].F));
            }
            // cout<<sum<<endl;
            continue;
        }
        ll a=(count[i].S.F)*(n-(count[i].S.S+count[i].F));
        if(i<count.size()-1){
            a*=(count[i+1].F-count[i].F);
        }
        a+=(count[i].S.F*(count[i].S.S)*(count[i].S.S+1))/2;
        sum+=a;
        // cout<<sum<<endl;
    }
    cout<<sum<<endl;

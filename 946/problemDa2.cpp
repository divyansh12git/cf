...
void solve(){
    ...
    string s;cin>>s;
    map<char,ll>count;
    count['N']=0;count['S']=0;count['E']=0;count['W']=0;
    for(auto i:s){
        count[i]++;
    }
    ll ns=(abs(count['N']-count['S']));
    ll ew=(abs(count['W']-count['E']));
    // for(auto k:count){
    //     cout<<k.first<<"->"<<k.second<<endl;
    // }
    if( n>2 && (ns%2!=0 || ew%2!=0)){
        tres(false);
        return;
    }
    if(n<=2 && (ns==0 && ew==0)){
    ...
    pll rov=make_pair(0,0),hel=make_pair(0,0);
    ll p1=0;
    string ans="";
    ll len=s.length();
    bool helip=false;
    while(p1<s.length()){
        ...
        if(p1<len && !helip){
            if(s[p1]=='N')hel.second++;
            else if(s[p1]=='S')hel.second--;
            else if(s[p1]=='W')hel.first--;
            else hel.first++;
            ans+='H';p1++;helip=true;
            // cout<<"NOOOOOOOO"<<endl;
            // cout<<hel.first<<"|"<<hel.second<<endl;
            continue;
        }
        if(s[p1]=='N'){
            ll ver=rov.second-hel.second;
            if(ver>0){
                ans+='H';hel.second++;
            }else{
                ans+='R';rov.second++;
    ...
    //   cout<<ans<<endl;
    if(hel.first==rov.first && hel.second==rov.second){
        cout<<ans<<endl;
    }else{
        cout<<"NO"<<endl;
    }

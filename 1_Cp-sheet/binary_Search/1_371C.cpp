...
bool pos(ll ham,ll nb,ll ns,ll nc,ll pbr,ll ps,ll pc,ll rb,ll rs,ll rc,ll ruble){
    nb-=ham*rb;
    ns-=ham*rs;
    nc-=ham*rc;
    ll cost=0;
...
void solve(){
    ...
    // cout<<r<<endl;
    // cout<<" "<<nb<<" "<<ns<<" "<<nc<<" "<<pbr<<" "<<ps<<" "<<pc<<" "<<rb<<" "<<rs<<" "<<rc<<" "<<r<<endl;
    ll lo=0,hi=1e14;
    ll ans=1e9;
    while(lo<=hi){
        ll mid=(lo+hi)>>1;
        
        bool p=pos(mid,nb,ns,nc,pbr,ps,pc,rb,rs,rc,r);

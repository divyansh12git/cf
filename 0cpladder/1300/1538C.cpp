...
void divyansh_8(){
    ...
    ll ans=0;
    fl(i,0,n){
        int x=l-a[i];//greater than this
        int y=r-a[i];//less than this
        auto lb=lower_bound(a.begin()+i+1,a.end(),x)-(a.begin()+i+1);
        auto ub=upper_bound(a.begin()+i+1,a.end(),y)-(a.begin()+i+1);
        // cerr<<lb<<" "<<ub<<endl;
        ans+=ub-lb;
    }
    cout<<ans<<endl;

...
void divyansh_8(){
    ...
    fl(i,0,n-1){
        ...
        vi temp;
        temp.pb(i+1);temp.pb(i+2);
        if(a[i]>a[i+1]){
            while(__gcd(ma,mi)!=1 || (i>0 && __gcd(ma,a[i-1])!=1)){
                ma++;
            }
            a[i]=ma;
        }else{
            while(__gcd(ma,mi)!=1 || (i<n-1 && __gcd(ma,a[i+1])!=1)){
                ma++;
            }
            a[i+1]=ma;
        }
        // cerr<<ma<<endl;
        temp.pb(a[i]);temp.pb(a[i+1]);
        ans.pb(temp);
    }
    // _print(a);
    cout<<ans.size()<<endl;
    for(auto it:ans){
        for(int x:it)cout<<x<<" ";cout<<endl;
    }

...
void divyansh_8(){
    ...
    while(m--){
        ...
        ll mincoins=INT64_MAX;
        // debug(*lb,*ub);
        if(lb!=a.end()){
            mincoins=min(mincoins,max(0LL,y-(sm-*lb)));
            // cerr<<mincoins<<" a"<<endl;
        }
        if(ub!=a.end()){
            mincoins=min(mincoins,max(0LL,y-(sm-*ub)));
            // cerr<<mincoins<<" b"<<endl;
        }
        if(lb!=a.begin())lb--;
        if(lb-a.begin()>=0){
            mincoins=min(mincoins,max(0LL,y-(sm-*lb))+max(0LL,x-*lb));
            // cerr<<mincoins<<" c"<<endl;
        }
        if(ub!=a.end())ub++;
        if(ub!=a.end()){
            mincoins=min(mincoins,max(0LL,y-(sm-*ub)));
            // cerr<<mincoins<<" d"<<endl;
        }
        cout<<mincoins<<endl;
    }
}

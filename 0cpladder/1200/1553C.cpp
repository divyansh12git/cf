...
int traverse(int ind, string &s, int a,int b,pii balls){
    if(ind>=s.length())return s.length();
    if(a>b+(5-balls.S) || a+(5-balls.F)<b)return ind;
    if(ind&1){
        balls.S++;
    }else balls.F++;
    if(s[ind]=='?'){
            return min(traverse(ind+1,s,((!(ind&1))?a+1:a),((ind&1)?b+1:b),balls),traverse(ind+1,s,a,b,balls));
    }else{
        if(s[ind]=='1'){
            if(ind&1)b++;
            else a++;
        }
        return traverse(ind+1,s,a,b,balls);
    }
}
...
void divyansh_8(){
    ...
    string s;
    cin>>s;
    cout<<traverse(0,s,0,0,{0,0})<<endl;

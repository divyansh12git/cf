...
void traverse(int node,vector<int>adj[],vi &vis,int pa,map<pii,int>& mp){
    vis[node]=1;
    for(auto & it:adj[node]){
        if(vis[it])continue;
        int res=2;
        if(pa!=-1 && mp[{node,pa}]==2){
            res=5;
        }
        mp[{node,it}]=res;
        mp[{it,node}]=res;
        traverse(it,adj,vis,node,mp);
    }
}
...
void divyansh_8(){
    ...
    vi vis(n+1,0);
    map<pii,int>mp;
    traverse(st,adj,vis,-1,mp);
    // _print(query);
    for(auto & it:query)cout<<mp[it]<<" ";cout<<endl;
}

...
// bool canPaint(int k,vpii vec){
//     vi col(k,0);
//     int p=0;
//     map<int,int>mp;
//     fl(i,0,vec.size()){
//         if(mp.find(vec[i].F)!=mp.end() && mp[vec[i].F]==k){
//             continue;
//         }
//         col[p%k]++;
//         mp[vec[i].F]++;
...
void divyansh_8(){
    ...
    fl(i,0,vec.size()){
        ...
        mp[vec[i].F]++;
        col[(p%k)+1].pb(vec[i].S);
        coloring[vec[i].S]=(p%k)+1;
        p++;
    }
    // _print(col);
    int mins=1e9;
    for(auto it:col){
        mins=min(mins,(int)(it.S.size()));
    }
    for(auto it:col){
        if(it.S.size()>mins){
            int rem=it.S.size()-mins;
            for(int i=0;i<rem;i++){
                coloring[it.S[i]]=0;
            }
        }

...
int traverse(map<int,vector<int>>tree,int x,vector<int>&a,int lv){
    if(tree.find(x)==tree.end())return a[x-1]/lv;
    int mini=INT_MAX;
    for(auto i:tree[x]){
       

        mini=min(mini,(a[x-1]/lv) + traverse(tree,i,a,lv+1));
        
       
    }
        
    // cout<<lv<<","<<a[x-1]<<"->"<<mini<<endl;

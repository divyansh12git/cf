...
bool isSorted(vector<ll> v) {fl(i,0,((int)v.size()) - 1) {if (v[i] > v[i + 1])return 0;}return 1;}
...
bool dfs(int r,int c,pii par,vector<string>& maze,vvi &vis,set<pii>&untrapped){
    int n=maze.size(),m=maze[0].size();
    // cerr<<r<<" "<<c<<endl;
    if(maze[r][c]=='?'){
        return 0;
    }
    if(untrapped.find({r,c})!=untrapped.end()){untrapped.insert({r,c});;return 1;}
    pii ins=getIns(maze[r][c]);
    pii nc={r+ins.F,c+ins.S};
    if(!isValid(nc.F,nc.S,n,m) ){
        untrapped.insert({r,c});
        return 1;
    }
    if(nc.F==par.F && nc.S==par.S)return 0;
    if(vis[r][c])return 0;
    vis[r][c]=1;
    // debug(nc.F,nc.S);
    bool res=dfs(nc.F,nc.S,{r,c},maze,vis,untrapped);
    if(res){
        untrapped.insert({r,c});
    }
    return res;
...
void divyansh_8(){
    ...
    fl(i,0,n){
        fl(j,0,m){
            if(!vis[i][j]){
                dfs(i,j,{-1,-1},maze,vis,untrapped);
            }
        }
    }
    // _print(untrapped);
    fl(i,0,n){
        fl(j,0,m){
            if(maze[i][j]=='?'){
                bool dne=0;
                fl(d,0,4){
                    pii nc={i+cor[d].F,j+cor[d].S};
                    if(!isValid(nc.F,nc.S,n,m))continue;
                    // cerr<<i<<" "<<j<<" "<<nc.F<<" "<<nc.S<<endl;
                    if(maze[nc.F][nc.S]=='?'){
                        maze[i][j]=dir[d];
                        maze[nc.F][nc.S]=opp[dir[d]];
                        dne=1;
                        break;
                    }
                    if(untrapped.find(nc)!=untrapped.end()){
                        continue;
                    }
                    maze[i][j]=dir[d];
                    dne=1;
                ...
                if(!dne){
                    untrapped.insert({i,j});
                }
            }
        }
    }
    // _print(maze);
    cout<<(n*m-untrapped.size())<<endl;
}

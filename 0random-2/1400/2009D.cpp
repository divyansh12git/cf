...
void divyansh_8(){
    ...
    fl(i,0,n){
        cin>>pts[i].F>>pts[i].S;
        fixy[pts[i].S].insert(pts[i].F);
        fixx[pts[i].F].pb(pts[i].S);
    }
    ll ans=0;
    for(auto& it:fixx){
        if(it.S.size()==2){
            ans+=fixy[0].size()+fixy[1].size()-2;
        }
        // cerr<<ans<<endl;
        int x=it.F;
        for(auto & y:it.S){
                // cerr<<y<<" "<<x+2<<" "<<y+1<<" "<<x+1<<endl;
                if(fixy[y].find(x+2)!=fixy[y].end() && (fixy[(y+1)%2].find(x+1)!=fixy[(y+1)%2].end())){
                    ans++;
                }

...
void solve(){
    ...
    fl(i,0,2*n)cin>>card[i];
    vector<pair<string, string>>ans;
    map<char,vi>mp;
    // _print(card);
    fl(i,0,2*n){
        mp[card[i][1]].pb(card[i][0]-'0');
    }
    for(auto& it:mp){
        vsort(it.S);
    }
    // _print(mp);
    for(auto& it:mp){
        if(it.F==tru)continue;
        if(it.S.size()>1){
            for(int i=0;i<it.S.size()-1;i+=2){
                string p1(1,it.F);
                string p2(1,it.F);
                ans.pb({to_string(it.S[i])+p1,to_string(it.S[i+1])+p2});
            }
            int rem=-1;
            if(it.S.size()&1){
                rem=it.S.back();
            ...
            it.S.clear();
            if(rem!=-1)it.S.pb(rem);
            // if(it.S.size()==0)mp.erase(it.F);
        }
    }
    if(mp.find(tru)!=mp.end()){
        vi trumph=mp[tru];
        int p=0;
        for(auto it:mp){
            if(it.F==tru)continue;
            if(it.S.size()>0){
                ...
                string p2(1,tru);
                if(p<trumph.size()){
                    ans.pb({to_string(it.S[0])+p1,to_string(trumph[p])+p2});
                    p++;
                }else{
        ...
        // _print(trumph);cerr<<endl;
        while(p<trumph.size()-1){
            string p1(1,tru);
            string p2(1,tru);
            ans.pb({to_string(trumph[p])+p1,to_string(trumph[p+1])+p2});
            p+=2;
        }
        if(p!=trumph.size()){
            cout<<"IMPOSSIBLE"<<endl;return;

...
ll fact[N+1];
ll modinv[N+1];
void precomp(){fact[0]=1;for(int i=1;i<=N;i++){fact[i]=(fact[i-1]*i)%mod;modinv[i]=expo(fact[i],mod-2,mod);}}
ll ncr(ll n,ll r){if(r>n)return 0;if(n==r)return 1;ll numo=fact[n];ll invdeno=(modinv[r]*modinv[n-r])%mod;ll ans=(numo*invdeno)%mod;return ans;}

...
bool bs(int d,string s,pii init,pii fin){
    int l=s.length();
    int x=d/l;
    cout<<x<<endl;
    pll mov={0,0};
    ...
    cout<<(mov.F);cout<<(" ");cout<<(mov.S);cout<<("\n");
    if(abs(init.F+mov.F-fin.F)+abs(init.S+mov.S-fin.S)<=d){
        return 1;
    }
    return 0;
}
...
void solve(){
    ...
    ll st=1,e=1e9;
    int ans=e;
    pii init={x1,y1},fin={x2,y2};
    while(st--){
        ll mid=(st+e)/2;
        cerr<<mid<<endl;
        if(bs(mid,s,init,fin)){
            ans=mid;
            e=mid-1;
        }else{
            s=mid+1;

...
void solve(){
    ...
    fl(i,0,n){
        vll temp;
        if(prev[i].F!=1e9){
            if(abs(prev[i].F-p[i])>abs(prev[i].S-p[i])){
                temp.pb(prev[i].F);
            }else{
                temp.pb(prev[i].S);
            }
        ...
        temp.pb(p[i]);
        if(next[i].F!=1e9){
            if(abs(next[i].F-p[i])>abs(next[i].S-p[i])){
                temp.pb(next[i].F);
            }else{
                temp.pb(next[i].S);
            }
        ...
        _print(temp);_print(ans);_print(vec);_print('\n');
        if(sm>=ans){
            ans=sm;
            vec.clear();
            vec=temp;
        ...
        if(temp.size()>2 && temp[0]==*(--(--vec.end())) && temp[1]==vec.back()){
            vec.pb(temp.back());
            ans+=abs(temp.back()-*(--(--temp.end())));
        }

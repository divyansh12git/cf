...
int parent(int node,vi &p){
    if(p[node]==node)return node;
    return p[node]=parent(p[node],p);
}
void unionBySize(int u,int v,vi & p,vi &deg){
    int uP=parent(u,p);
    int vP=parent(v,p);
    if(uP==vP)return;
    else {
...
void solve(){
    ...
    fl(i,0,m){
        ...
        fl(i,1,k){
            int a;
            cin>>a;
            unionBySize(q,a,par,deg);
        }
    }
    // _print(par);
    fl(i,1,n+1){
        cout<<deg[parent(i,par)]<<" ";
    }cout<<endl;
}

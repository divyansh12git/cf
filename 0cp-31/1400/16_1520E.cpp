...
void solve(){
    ...
    if(sp>0){
        if(s[p2]=='*' )sheeps.pb(sp);
        else spaces.pb(sp);
    }
    if(spaces.size()==0 || sheeps.size()==0){
        cout<<0<<endl;return;
    }
    // _print(sheeps);_print(spaces);
    vll pre(sheeps.size(),0);
    ll prev=sheeps[0];
    fl(i,1,sheeps.size()){
        pre[i]=pre[i-1]+prev*spaces[i-1];
        prev+=sheeps[i];
    }
    ll com=sheeps.back();
    vll suff(sheeps.size(),0);
    for(int i=sheeps.size()-2;i>=0;i--){
        suff[i]=suff[i+1]+com*spaces[i];
        com+=sheeps[i];
    }
    ll ans=1e18;

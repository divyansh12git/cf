...
void solve(){
    ...
    // for(auto it:col){
    //     cerr<<it.F<<"->";
    //     for(auto x:it.S)cerr<<x.F<<" "<<x.S<<",";cerr<<endl;
    // }
    ll ans=0;
    for(auto it:col){
        vector<pll>v=it.S;
        ...
        vsort(x);vsort(y);
        vll suffx(x.size()+1),suffy(y.size()+1);
        suffx[x.size()]=0;suffy[y.size()]=0;
        for(int i=x.size()-1;i>=0;i--){
            suffx[i]=suffx[i+1]+x[i];
            suffy[i]=suffy[i+1]+y[i];
        }
        // _print(x);_print(y);_print(suffx);_print(suffy);cerr<<endl;
        fl(i,0,x.size()){
            ans+=(suffx[i+1]-(x.size()-i-1)*x[i]);
            ans+=(suffy[i+1]-(x.size()-i-1)*y[i]);
        }

...
void solve(){
    ...
    sort(all(pos),revsort);
    sort(all(negs),revsort);
    ll check=abs(*min_element(all(a))-*(max_element(all(a))));
    bool status=1;
    int p1=0,p2=0;
    ll sm=0;
    vll ans;
    ...
    tres(status);
    if(status){fl(i,0,ans.size())cout<<ans[i]<<" ";cout<<endl;}
}

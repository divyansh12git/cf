...
void solve(){
    ...
    //sorting the string based on frequency:
    sort(temp.begin(),temp.end(),[&](char &a,char &b){
        return freq[a]>freq[b];
    });
    // cerr<<temp<<endl;
    int pos=0,diff=INT_MAX,perchar=0;
   
    //iterating over all the unique characters available in lowecase
    for(int f=1;f<=26;f++){
        if(n%f!=0)continue;
        int numperchar=n/f;
        vector<int>changes(26);
        fl(i,0,f){
            changes[temp[i]-'a']=abs(freq[temp[i]]-numperchar);
        }
        fl(i,f,26){
            changes[temp[i]-'a']=freq[temp[i]];
        }
        int tot=accumulate(all(changes),0);
        if(tot<diff){
            diff=tot;
            pos=f;
            perchar=numperchar;
    ...
    int p1=0,p2=pos-1,p3=temp.length()-1;
    //removing the unwanted characters with wanted characters;
    while(p3>=pos ){
        if(freq[temp[p3]]>0){
            int ind=loc[temp[p3]].back();
            loc[temp[p3]].pop_back();
            s[ind]=temp[p2];
            freq[temp[p2]]++;
            freq[temp[p3]]--;
            if(freq[temp[p2]]==perchar)p2--;
            if(freq[temp[p3]]==0)p3--;
        }else p3--;
    }
    
    //removing extra characters from the string
    while(p2>p1){
        if(freq[temp[p1]]==perchar)p1++;
        else if(freq[temp[p2]]==perchar)p2--;
        else{
            int ind=loc[temp[p1]].back();
            // cerr<<ind<<endl;
            loc[temp[p1]].pop_back();
            s[ind]=temp[p2];

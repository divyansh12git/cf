...
void solve(){
    ...
    fl(i,0,n){
        if(a[i]=='L')log.pb(i+1);
        if(a[i]=='W')water.pb(i+1);
    }
    ll p1=0,p2=0;
    ll i=0;
    // cout<<"IHI"<<endl;
    while(i<n+1){
        while(p1<log.size() && i+m>=log[p1]){
            i=log[p1];
            p1++;
        }
        if(i+m>n)break;
        while(p2<water.size() && water[p2]<=i)p2++;
        ll ju=p2;
        while(ju+1<water.size() && water[++ju]<=i+m){
            p2++;
        }
        i=water[p2];
        while(i<n+1 && k>0){
            if(a[i-1]=='L'){p1++;i++;break;}
            if(a[i-1]=='C'){ans=false;break;}
            k--;i++;
            if(p2<water.size())p2++;
        }
        if(k==0)ans=false;
        if(i==n+1 || ans==false)break;
    }
    tres(ans);
}

...
void solve(){
    ...
    int n,w;
    cin>>n>>w;
    vpii knapsack(n+1);//weight and value
    fl(i,1,n+1){
        cin>>knapsack[i].F>>knapsack[i].S;
    }
    ll dp[n+1][w+1];
    // the maximum value of items from 0...i taken such that the weight of the bag is j;
    fl(i,0,n+1){
        fl(j,0,w+1){
            dp[i][j]=0;
            if(i>0){
                ll op1=dp[i-1][j],op2=0;
                if(j>=knapsack[i].F){
                    op2=dp[i-1][j-knapsack[i].F]+knapsack[i].S;
                }
                dp[i][j]=max(op1,op2);
            }
            // cerr<<dp[i][j]<<" ";
